import json
import paho.mqtt.client as mqtt
import serial
import time

POINT_PER_PACK = 12
PACKAGE_SIZE = 47
HEADER = 0x54

CrcTable = [  ]

def CalCRC8(p):
    crc = 0
    for val in p:
        crc = CrcTable[(crc ^ val) & 0xff]
    return crc

def AssignValues(package, client):
    frame = {
        'header': package[0],
        'ver_len': package[1],
        'speed': (package[3] << 8 | package[2]),
        'start_angle': (package[5] << 8 | package[4]) / 100,
        'end_angle': (package[43] << 8 | package[42]) / 100,
        'timestamp': (package[45] << 8 | package[44]),
        'object_within_2m': 0,
        'crc8': package[46],
        'point': [None] * POINT_PER_PACK
    }

    angle_step = (frame['end_angle'] - frame['start_angle']) / 11
    AssignValues.count += 1

    for i in range(12):
        distance = (package[8 + i * 3 - 1] << 8 | package[8 + i * 3 - 2])
        confidence = package[8 + i * 3]
        angle = frame['start_angle'] + angle_step * i

        frame['point'][i] = {'distance': distance, 'angle': angle, 'confidence': confidence}

        if confidence >= 180 and 0 <= angle <= 15 or 345 <= angle <= 360:
            if distance <= 480:
                frame['object_within_2m'] = 1
               # print(f"Point {i}, Distance: {distance}, Angle = {angle}")

    point_to_publish = frame['point'][0]
    if frame['object_within_2m']:
        if AssignValues.count >= 100:
            AssignValues.count = 0
            # Publish a message to the MQTT broker
            # for point in frame['point']:
            message = {
               "angle": point_to_publish['angle'],
               "distance": point_to_publish['distance']
            }
            client.publish(TOPIC, json.dumps(message), qos=0)
            time.sleep(1.4)
            # print(f"Published message to topic {TOPIC}: {message}")
    return frame

BROKER = "localhost"
PORT = 1883
TOPIC = "lidar/data"
AssignValues.count = 0

def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print("Connected to MQTT Broker!")
    else:
        print("Failed to connect, return code %d\n", rc)

def main():
    # Create a MQTT client
    client = mqtt.Client()

    # Assign the callback function
    client.on_connect = on_connect

    # Connect to the Broker
    client.connect(BROKER, PORT, 60)

    # Start the MQTT client
    client.loop_start()

    ser = serial.Serial(port='/dev/ttyUSB0', baudrate=230400, bytesize=8, parity='N', stopbits=1, timeout=1)

    package = [0] * PACKAGE_SIZE
    packageIndex = 0

    while True:
        data = ser.read(256)
        if len(data) == 0:
            print("No data read from the serial port")
            continue

        for byte in data:
            if packageIndex == 0 and byte != HEADER:
                continue

            package[packageIndex] = byte
            packageIndex += 1

            if packageIndex == PACKAGE_SIZE:
                crc = CalCRC8(package[:PACKAGE_SIZE - 1])
                if crc == package[PACKAGE_SIZE - 1]:
                    AssignValues(package, client)
                    packageIndex = 0

                else:
                    packageIndex = 0

    # Stop the MQTT client
    client.loop_stop()

if __name__ == "__main__":
    main()
default@raspberrypi:~/objectdetection $ cat publisher.py
import json
import paho.mqtt.client as mqtt
import serial
import time

# code imported from LD-19 LiDAR Manual

POINT_PER_PACK = 12
PACKAGE_SIZE = 47
HEADER = 0x54

CrcTable = [  ]

def CalCRC8(p):
    crc = 0
    for val in p:
        crc = CrcTable[(crc ^ val) & 0xff]
    return crc

def AssignValues(package, client):
    frame = {
        'header': package[0],
        'ver_len': package[1],
        'speed': (package[3] << 8 | package[2]),
        'start_angle': (package[5] << 8 | package[4]) / 100,
        'end_angle': (package[43] << 8 | package[42]) / 100,
        'timestamp': (package[45] << 8 | package[44]),
        'object_within_2m': 0,
        'crc8': package[46],
        'point': [None] * POINT_PER_PACK
    }

    angle_step = (frame['end_angle'] - frame['start_angle']) / 11
    AssignValues.count += 1

    for i in range(12):
        distance = (package[8 + i * 3 - 1] << 8 | package[8 + i * 3 - 2])
        confidence = package[8 + i * 3]
        angle = frame['start_angle'] + angle_step * i

        frame['point'][i] = {'distance': distance, 'angle': angle, 'confidence': confidence}
        # makes sure confidence value is higher than or equal to 180 and only in the 30 degrees infront of the car
        if confidence >= 180 and 0 <= angle <= 15 or 345 <= angle <= 360:
            # makes sure distance is 48 cm away from car for stopping distance
            if distance <= 480:
                frame['object_within_2m'] = 1
    point_to_publish = frame['point'][0]
    if frame['object_within_2m']:
        if AssignValues.count >= 100:
            AssignValues.count = 0
            # Publish a message to the MQTT broker (only the first frame not all 12)
            message = {
               "angle": point_to_publish['angle'],
               "distance": point_to_publish['distance']
            }
            client.publish(TOPIC, json.dumps(message), qos=0)
            # sleep so car can preform object avoidance actions (not needed for braking/cruise control)
            time.sleep(1.4)
    return frame

# connecting to mqtt broker Mosquitto

BROKER = "localhost"
PORT = 1883
TOPIC = "lidar/data"
AssignValues.count = 0

def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print("Connected to MQTT Broker!")
    else:
        print("Failed to connect, return code %d\n", rc)

def main():
    # Create a MQTT client
    client = mqtt.Client()

    # Assign the callback function
    client.on_connect = on_connect

    # Connect to the Broker
    client.connect(BROKER, PORT, 60)

    # Start the MQTT client
    client.loop_start()

    ser = serial.Serial(port='/dev/ttyUSB0', baudrate=230400, bytesize=8, parity='N', stopbits=1, timeout=1)

    package = [0] * PACKAGE_SIZE
    packageIndex = 0
    # reading data from LiDAR
    while True:
        data = ser.read(256)
        if len(data) == 0:
            print("No data read from the serial port")
            continue

        for byte in data:
            if packageIndex == 0 and byte != HEADER:
                continue

            package[packageIndex] = byte
            packageIndex += 1

            if packageIndex == PACKAGE_SIZE:
                crc = CalCRC8(package[:PACKAGE_SIZE - 1])
                if crc == package[PACKAGE_SIZE - 1]:
                    AssignValues(package, client)
                packageIndex = 0

    # Stop the MQTT client
    client.loop_stop()

if __name__ == "__main__":
    main()
